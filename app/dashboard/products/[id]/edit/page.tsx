...
const schema = object({
    ...
    WarrantyDetails: string().min(1, {
      message: 'Warranty details are required.',
    }),
      ...
      try {
        ...
        if (token) {
          const response = await fetch(
            `https://insurance-claim-server.vercel.app/api/product/getProductById/${params.id}`,
            {
              method: 'GET',
              headers: {
    ...
    const getProduct = async () => {
      ...
      if (productResponse !== null) {
        const updatedFormData = {
          ...
          WarrantyDetails: productResponse.WarrantyDetails,
          Availability: productResponse.Availability,
        };
  ...
  const onSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      ...
      const response = await axios.put(
        'https://insurance-claim-server.vercel.app/api/product/update/'+ params.id ,
    ...
    <form onSubmit={onSubmit}>
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        <div className="mb-4">
          ...
          {formErrors['Model'] && (
            <p className="mb-1 text-sm text-red-500">{formErrors['Model']}</p>
          )}
        </div>
        ...
        <div className="mb-4">
          ...
          {formErrors['Price'] && (
            <p className="mb-1 text-sm text-red-500">{formErrors['Price']}</p>
          )}
        </div>
